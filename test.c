           ...
           for(l=0;l<6;l++){
                if(m.mat[l][position]!='x' && m.mat[l][position]!='o' ){
                   ...
                   if( l==5 ){
                        ...
                        m.mat[5][position]=.symbole;gotoxy(newx-1,newy);printf("%c",m.mat[l][position]);
                }}}
                    else{ if(m.mat[0][position]!='x' && m.mat[0][position]!='o'){
                            for(l=0;l<6;l++){
                ...
                if(m.mat[l][position]!='x' && m.mat[l][position]!='o' ){
                   ...
                   if(m.mat[l][position]=='x' || m.mat[l][position]=='o'  ){  if(m.mat[l-1][position]!='x' && m.mat[l-1][position]!='o'  ){
                        m.mat[l-1][position]=.symbole;gotoxy(newx-1,newy);printf("%c",m.mat[l-1][position]);
   ...
   for(l=0;l<6;l++){
    ...
    if(m.mat[l][position]==symbole ){
        compt++;
                ...
                if(compt==4 && .symbole==symbole){gotoxy(8,27);printf("Victoire sur la Verticale de %s " , .joueur);return 1 ;break;}
...
compt=0;
        ...
        for(c=0;c<7;c++){
            if(m.mat[l][c]==symbole ){
                ...
                if(compt==4 && .symbole==symbole){gotoxy(8,27);printf("Victoire sur l'Horizontale de %s " , .joueur);return 1 ;break;}
...
// Diagonale gauche
    ...
    if(m.mat[l][position]==symbole ){
        ...
        if(compt==4 && jou1.symbole==symbole){gotoxy(8,27);printf("Victoire sur la Diagonale Gauche Vers le Bas de %s " , jou1.joueur);return 1 ;break;}
                if(compt==4 && .symbole==symbole){gotoxy(8,27);printf("Victoire sur la Diagonale Gauche vers le Bas de %s " , .joueur);return 1 ;break;}
        }
//Diagonale Droite
          compt=0;position=d;
    ...
    if(m.mat[l][position]==symbole ){
        ...
        if(compt==4 && jou1.symbole==symbole){gotoxy(8,27);printf("Victoire sur la Diagonale Droite Vers le Bas de %s " , jou1.joueur);return 1 ;break;}
                if(compt==4 && .symbole==symbole){gotoxy(8,27);printf("Victoire sur la Diagonale Droite Vers le Bas de %s " , .joueur);return 1 ;break;}
        }
//Diagonale haut Droite
    ...
    if(m.mat[l][position]==symbole ){
        ...
        if(compt==4 && jou1.symbole==symbole){gotoxy(8,27);printf("Victoire sur la Diagonale Droite Vers le Haut de %s " , jou1.joueur);return 1 ; break;}
                if(compt==4 && .symbole==symbole){gotoxy(8,27);printf("Victoire sur la Diagonale Droite Vers le Haut  de %s " , .joueur);return 1 ;break;}
        }
...
//Diagonale haut Gauche
    ...
    if(m.mat[l][position]==symbole ){
        ...
        if(compt==4 && jou1.symbole==symbole){gotoxy(8,27);printf("Victoire sur la Diagonale Gauche Vers le Haut de %s " , jou1.joueur);return 1;break;}
                if(compt==4 && .symbole==symbole){gotoxy(8,27);printf("Victoire sur la Diagonale Gauche Vers le Haut de %s " , .joueur);return 1 ;break;}
        }
...
void init_jeu(){
     char p1 ; char p2;int verif=0; int taille; int tour;int victoire=0;
        ...
        gotoxy(37+taille,25);scanf(" %c", &jou1.symbole);
         ...
         gotoxy(8,25);printf("%s choisi ton symbole (x/o):", .joueur);taille = strlen(.joueur);
        gotoxy(37+taille,25);scanf(" %c", &.symbole);
        }

        if(jou1.symbole!='x' && jou1.symbole!='o'&& .symbole!='x' && .symbole!='o'){
            gotoxy(8,26);printf("Erreur choisi ton symbole (x/o)");
        }else{verif=1;if(jou1.symbole=='x'){.symbole='o';}if(jou1.symbole=='o'){.symbole='x';}if(.symbole=='x'){jou1.symbole='o';}if(.symbole=='o'){jou1.symbole='x';}}
        }while(verif==0);
        clrscr();grille4puissance();

        gotoxy(80,8);printf("JEU DE PUISSANCE 4");
        gotoxy(80,12);printf("Symbole de %s  : %c" , jou1.joueur, jou1.symbole);
        gotoxy(80,13);printf("Symbole de %s  : %c" , .joueur , .symbole);
             if(random==1){   gotoxy(80,17);printf("C'est  %s qui commence.", jou1.joueur);}else{gotoxy(80,17);printf("C'est  %s qui commence.", .joueur);}


             tour=random;int position1 ,position2;
            do{
               if(tour==1 && victoire!=1){
               do{ gotoxy(8,27);printf("  %s Entre une position valide:", jou1.joueur); gotoxy(38+taille,27);scanf("%d", &position1);}while(position1<0 ||position1>6);
               jouer_colonne(tour,position1);victoire=verif_gain(jou1.symbole,position1);tour=2;}

                if(tour==2 && victoire!=1){
                 do{ gotoxy(8,27);printf("  %s Entre une position valide:", .joueur); gotoxy(38+taille,27);scanf("%d", &position2);}while(position2<0 ||position2>6);
                 jouer_colonne(tour,position2);victoire=verif_gain(.symbole,position2);tour=1;}
